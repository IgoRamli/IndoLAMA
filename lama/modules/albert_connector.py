...
from transformers import BertTokenizer, AlbertForMaskedLM, BasicTokenizer, BertModel
import numpy as np
...
class Albert(Base_Connector):
    ...
    def __init__(self, args, vocab_subset = None):
        ...
        if args.bert_model_dir is not None:
            ...
            print("loading ALBERT model from {}".format(bert_model_name))
        else:
        ...
        self.masked_bert_model = AlbertForMaskedLM.from_pretrained(bert_model_name)
    ...
    def __get_input_tensors(self, sentences):
        ...
        if len(sentences) > 2:
            print(sentences)
            raise ValueError("ALBERT accepts maximum two sentences in input for each data point")
