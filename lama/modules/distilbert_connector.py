...
#
import torch
import transformers as btok
from transformers import DistilBertTokenizer, DistilBertForMaskedLM, BasicTokenizer
import numpy as np
...
class DistilBert(Base_Connector):
    ...
    def __init__(self, args, vocab_subset = None):
        ...
        self.distilbert_model = self.masked_distilbert_model.distilbert
    ...
    def __get_input_tensors(self, sentences):
        ...
        if len(sentences) > 2:
            print(sentences)
            raise ValueError("DistilBERT accepts maximum two sentences in input for each data point")
    ...
    def _cuda(self):
        self.masked_distilbert_model.cuda()
    ...
    def get_batch_generation(self, sentences_list, logger= None,
        ...
        with torch.no_grad():
            output = self.masked_distilbert_model(
    ...
    def get_contextual_embeddings(self, sentences_list, try_cuda=True):
        ...
        with torch.no_grad():
            all_encoder_layers, _ = self.distilbert_model(
